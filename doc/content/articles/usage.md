...
minimega uses a readline-like command prompt for input. Just like bash,
minimega supports tab completion on commands and paths, line editing via
emacs-like bindings, and history. Commands are input simply as
`<command> <arguments>`, with arguments split on whitespace, and grouped by ""
or '', just like bash.
...
minimega can generate a tab-completion script for `bash`, `zsh`, or `fish`
for its command line flags, as well as for minimega commands typed after
the `e` or `attach` flags:

```text
$ minimega -completion bash
$ minimega -completion zsh
$ minimega -completion fish
```
...
To enable it for every session, write the script somewhere your shell's
completion system will find it. For bash, that's typically:
...
For zsh, write it somewhere in your `fpath` (a directory searched by
`compinit`), e.g.:
...
For fish, write it to fish's vendor completions directory, e.g.:

```text
$ minimega -completion fish | sudo tee /usr/share/fish/vendor_completions.d/minimega.fish >/dev/null
```

With completion installed, typing `minimega -e vm sta<TAB>` at the shell
prompt will complete to `minimega -e vm start`, the same way tab completion
works at the minimega command prompt itself.

minimega packages (RPM and Debian) install the bash completion script
automatically to `/usr/share/bash-completion/completions/minimega` as part
of a standard package install; see the packaging scripts under `packaging/`
for details.
...
[minirouter](router.md) provides more advanced routing, DHCP,
...
minimega includes a standalone webserver called `miniweb` that users can use to
view VM state and connect to running VMs. See the
[miniweb](miniweb.md) article for more information.
