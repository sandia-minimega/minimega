...
![The VM list at /vms](miniweb/mw_vmi.png)

The buttons above the table toggle columns, and the Search box filters rows
as you type. **Toggle top view** switches the same page to `vm top`, which
refreshes and shows memory, CPU, process count, and interface counters:

![The top view at /vms#top](miniweb/mw_vmt.png)

!!! note
    The screenshots on this page were captured on an earlier release. The
    layout and the tables are the same today, with two differences: the
    navigation bar has gained a Builder entry for `/minibuilder`, and the VM
    table follows the current `vm info` columns.

### The other pages

`/tilevnc` draws one live screenshot per VM and connects you to a VM when you
click its tile, which is the quickest way to watch a whole experiment:

![The screenshot grid at /tilevnc](miniweb/mw_ss.png)

`/hosts` is the `host` table, one row per node, with the resource and commit
columns that the scheduler uses:

![The host table at /hosts](miniweb/mw_h.png)

`/graph` draws the VMs and the VLANs that join them, with a sidebar listing
the members of each VLAN:

![The network graph at /graph](miniweb/mw_g.png)

`/files/` lists the files directory of the namespace and serves a file when
you click it:

![The file listing at /files/](miniweb/mw_f.png)

`/namespaces` lists every namespace with its VLAN range, and links to that
namespace's own pages:

![The namespace list at /namespaces](miniweb/mw_n.png)
...
![A KVM VM's console at /vm/&lt;name&gt;/connect/](miniweb/mw_vnc.png)

The noVNC toolbar at the top right scales the display, goes full screen, and
sends key combinations the browser would otherwise intercept.
...
![The browser console at /console](miniweb/mw_c.png)
