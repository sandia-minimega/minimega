...
script, `collect.bash`, writes a file we then fetch:

```bash
#!/bin/bash
ip address > /var/log/inventory.out
```
...
`<filepath>/miniccc_responses/3/<uuid>/var/log/inventory.out`.
...
The example starts a web server inside the VM, forwards a host port to it,
and then fetches the page through the tunnel with `shell curl`. The listening
port is opened on the host that runs the VM, so on a cluster run the client
there rather than on the head node.
