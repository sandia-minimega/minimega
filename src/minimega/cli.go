...
func cliCompleter(line string) []string {
	prep := func(s []string) []string {
		...
		// create new result that is line + whitespace + suggestion
		r := make([]string, len(s))
		for i := range s {
			r[i] = line + s[i]
		}
...
func cliLocal() {
	rl, err := readline.NewEx(&readline.Config{
		AutoComplete: readline.NewPrefixCompleter(
			readline.PcItemDynamic(cliCompleter),
		),
	})
	if err != nil {
		log.Fatalln(err)
	...
	for {
		...
		if namespace == "" {
			rl.SetPrompt("minimega$ ")
		} else {
			rl.SetPrompt(fmt.Sprintf("minimega[%v]$ ", namespace))
		}
		...
		log.Debug("got from stdin: `%v`", line)
