...
func init() {
	...
	cliCommands = map[string]*command{
		"rate": &command{
			...
			Helplong: `
> rate [launch rate in milliseconds]
...
'help', do not get recorded.`,
		...
		"vm_save": &command{
			...
			Helplong: `
> vm_save <save name> <vm name/id> [<vm name/id> ...]
...
the file in manually.`,
		...
		"vm_info": &command{
			...
			Helplong: `
> vm_info [output=<quiet,json>] [search=<search term>] [ [output mask[,...]] ]
...
An alias to 'quit'.`,
		...
		"vm_launch": &command{
			...
			Helplong: `
> vm_launch <number of vms or vm name>
...
to be the boot device.`,
		...
		"vm_kernel": &command{
			...
			Helplong: `
> vm_kernel [path to kernel image]
...
For example, to set a static IP for a linux VM:
	vm_append ip=10.0.0.5 gateway=10.0.0.1 netmask=255.255.255.0 dns=10.10.10.10`,
		...
		"vm_net": &command{
			...
			Helplong: `
> vm_net [bridge,]<id>[,<mac address>][,<driver>] [[bridge,]<id>[,<mac address>][,<driver>] ...]
...
commands.`,
		...
		"host_tap": &command{
			...
			Helplong: `
> host_tap [<create [bridge] vlan <A.B.C.D/MASK,dhcp,none>, delete <tap name>]
...
response.`,
		...
		"mesh_set": &command{
			...
			Helplong: `
> mesh_set [annotate] <recipients> <command>
...
prepending the keyword 'annotate' to the command:
	...
	mesh_set annotate kn[1-2] vm_info`,
		...
		"mesh_broadcast": &command{
			...
			Helplong: `
> mesh_broadcast [annotate] <command>
...
Return the hostname.`,
		...
		"dnsmasq": &command{
			...
			Helplong: `
> dnsmasq [start [<listen address> <DHCP low range> <DHCP high range> [config file], config file], kill <id>]
...
allows a single disk image to be used for many VMs.`,
		...
		"optimize": &command{
			...
			Helplong: `
> optimize [ksm [true,false], hugepages [path], affinity [true,false]]
...
Display the version.`,
		...
		"vm_config": &command{
			...
			Helplong: `
> vm_config [save <name>, restore <name>]
...
of VMs that have been flushed may be reused.`,
		...
		"file": &command{
			...
			Helplong: `
> file <list [path], get <file>, delete <file>, status>
...
specified by the -filepath flag. The filename will be returned.`,
		...
		"vm_hotplug": &command{
			...
			Helplong: `
> vm_hotplug [add <id> <filename>, remove <id> <file id>]
...
To remove all hotplug devices, use ID -1.`,
		...
		"vm_netmod": &command{
			...
			Helplong: `
> vm_netmod <vm name or id> <tap position> <vlan,disconnect>
...
the backing qcow image for a snapshot image.`,
		...
		"define": &command{
			...
			Helplong: `
> define [macro[(<var1>[,<var2>...])] <command>]
