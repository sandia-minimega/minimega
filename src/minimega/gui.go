...
func guiHome() string {
	return fmt.Sprintf(HTMLFRAME, "Home")
}
...
func guiMapVMs(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte(fmt.Sprintf(HTMLFRAME2, d3head, d3body)))
}
...
func guiStats(w http.ResponseWriter, r *http.Request) {
	...
	if len(s) != 0 { //check if there are other hosts
		for _, resp := range s { //[0].Tabular { //mesh data
			tl := `<tr>`
			for _, entry := range resp.Tabular {
...
func guiAllVMs(writer http.ResponseWriter, request *http.Request) {
	...
	maskL := "id,name,state,memory,vcpus,disk,initrd,kernel,cdrom,mac,ip,vlan,append,tags"
	mask := "id,host,name,state,memory,vcpus,disk,initrd,kernel,cdrom,mac,ip,vlan,append,tags"
...
func guiHostVMs(host string) string {
	...
	mask := "id,name,state,memory,vcpus,disk,initrd,kernel,cdrom,mac,ip,vlan,append,tags"
