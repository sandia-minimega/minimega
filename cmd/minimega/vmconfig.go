...
func (old BaseConfig) Copy() BaseConfig {
	...
	res.Bonds = make(map[string]BondConfig)
	for k, v := range old.Bonds {
		res.Bonds[k] = v.Copy()
	}
...
func (vm *BaseConfig) BondString() string {
	bonds := []string{}

	for k, v := range vm.Bonds {
		ifaces := make([]string, len(v.Interfaces))

		for i, c := range v.Interfaces {
			ifaces[i] = c.Tap
		}

		bonds = append(bonds, fmt.Sprintf("%s (%s)", k, strings.Join(ifaces, " ")))
	}

	return fmt.Sprintf("[%s]", strings.Join(bonds, " "))
}
