...
var (
	...
	f_completion  = flag.String("completion", "", "print a tab-completion script for the given shell (bash, zsh, fish) to stdout and exit")
...
func main() {
	...
	// generates a tab-completion script from the flags registered above,
	// so this needs to happen after flag.Parse() but doesn't depend on
	// anything else minimega sets up
	if *f_completion != "" {
		script, err := shellCompletion(*f_completion)
		if err != nil {
