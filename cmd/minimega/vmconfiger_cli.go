...
`,
	...
	},
	{
		...
		HelpLong: `Assign a state image, generated by a previously saved VM to boot
...
`,
	...
	{
		HelpShort: "configures snapshot-first-disk-only",
		HelpLong: `Only snapshot the first disk. When enabled, only the first disk will have a
...
`,
		Patterns: []string{
			"vm config snapshot-first-disk-only [true,false]",
		},
		Call: wrapSimpleCLI(func(ns *Namespace, c *minicli.Command, r *minicli.Response) error {
			if len(c.BoolArgs) == 0 {
				r.Response = strconv.FormatBool(ns.vmConfig.SnapshotFirstDiskOnly)
				return nil
			}

			ns.vmConfig.SnapshotFirstDiskOnly = c.BoolArgs["true"]
...
`,
	...
	},
	{
		...
		Patterns: []string{
			...
			"clear vm config <snapshot-first-disk-only,>",
...
func (v *KVMConfig) Info(field string) (string, error) {
	...
	if field == "snapshot-first-disk-only" {
		return strconv.FormatBool(v.SnapshotFirstDiskOnly), nil
	}
...
func (v *KVMConfig) Clear(mask string) {
	...
	if mask == Wildcard || mask == "snapshot-first-disk-only" {
		v.SnapshotFirstDiskOnly = false
	}
...
func (v *KVMConfig) WriteConfig(w io.Writer) error {
	...
	if v.SnapshotFirstDiskOnly != false {
		fmt.Fprintf(w, "vm config snapshot-first-disk-only %t\n", v.SnapshotFirstDiskOnly)
	}
...
func (v *KVMConfig) ReadConfig(r io.Reader, ns string) error {
	...
	for scanner.Scan() {
		...
		case "snapshot-first-disk-only":
			v.SnapshotFirstDiskOnly, _ = strconv.ParseBool(config[1])
		case "usb-use-xhci":
